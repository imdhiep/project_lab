...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Register the BDD100K scene baseline used by the person-query stack.")
    parser.add_argument(
        "--config",
        default=str(Path(__file__).resolve().parents[1] / "configs" / "person_query_stack" / "scene_model.json"),
    )
    return parser
...
def main() -> int:
    ...
    print(f"Wrote scene baseline artifact to {output_artifact}")
    return 0
